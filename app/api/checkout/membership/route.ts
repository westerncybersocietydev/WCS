...
export async function POST(req: Request) {
  try {
    ...
    // DEV MODE - Check if Stripe is configured
    if (
      process.env.NODE_ENV === "development" &&
      !process.env.STRIPE_SECRET_KEY
    ) {
      console.log(
        "🚀 DEV MODE: Stripe not configured - please set up your .env.local file"
      );
      return NextResponse.json(
        {
          error:
            "Stripe not configured. Please set up your .env.local file with Stripe test keys.",
        },
        { status: 500 }
      );
    }
