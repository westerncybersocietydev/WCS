...
function TicketConfirmContent() {
  ...
  const formatDateTimeForGoogle = (dateStr: string, timeStr: string) => {
    try {
      // Parse date (e.g., "Saturday, January 24, 2026")
      ...
      let modifier: string;
      if (timeStr.includes(" - ")) {
        [timePart, modifier] = timeStr.split(" - ")[0].trim().split(" ");
      } else {
        [timePart, modifier] = timeStr.split(" ");
      }
      ...
      const yearPart = parseInt(year);
      const monthPart = String(monthNum).padStart(2, "0");
      const dayPart = String(parseInt(dayStr)).padStart(2, "0");
      const hoursPart = String(hour24).padStart(2, "0");
  ...
  const fetchTicketDetails = useCallback(async (userId: string, eventId: string) => {
    try {
      ...
      if (eventResponse.ok) {
        ...
        setEventName(eventData.name || "Lost Love Bar Night");
        ...
        if (eventData.date && eventData.time && eventData.location) {
          try {
            ...
            const dayStr = dateParts[2].replace(",", "").replace(/st|nd|rd|th/g, "");
            ...
            const startDateTime = formatDateTimeForGoogle(eventData.date, startTimeStr.trim());
            
            // Calculate end time - default to 3 hours later for bar night
            let endDateTime: string;
            if (endTimeStr) {
              const endDateTimeFull = formatDateTimeForGoogle(eventData.date, endTimeStr.trim());
              endDateTime = endDateTimeFull;
            } else {
              // Default to 3 hours later for bar night
              const [hours, minutes] = startTimeStr.trim().split(" ")[0].split(":").map(Number);
              const modifier = startTimeStr.trim().split(" ")[1];
              let hour24 = hours;
              if (modifier === "PM" && hours !== 12) hour24 += 12;
              if (modifier === "AM" && hours === 12) hour24 = 0;
              ...
              const monthNum = monthNames.indexOf(month) + 1;
              const endDate = new Date(parseInt(year), monthNum - 1, parseInt(dayStr), hour24, minutes || 0);
              endDate.setHours(endDate.getHours() + 3); // 3 hours for bar night
              
              const yearPart = endDate.getFullYear();
              const monthPart = String(endDate.getMonth() + 1).padStart(2, "0");
              const dayPart = String(endDate.getDate()).padStart(2, "0");
              const hoursPart = String(endDate.getHours()).padStart(2, "0");
              const minutesPart = String(endDate.getMinutes()).padStart(2, "0");
              endDateTime = `${yearPart}${monthPart}${dayPart}T${hoursPart}${minutesPart}00`;
            }
  ...
  useEffect(() => {
    ...
    if (!ticketNum) {
      router.push("/lost-love-bar-night");
      return;
    }
      ...
      <>
        ...
        <div className="flex items-center justify-center min-h-screen">
          <div className="text-center">
            ...
            <p className="mt-2 text-gray-600">Loading wristband...</p>
          </div>
        </div>
        <Footer />
      </>
            ...
            <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center">
              <div className="text-white text-center px-6 py-12 max-w-3xl mx-auto">
                ...
                <p className="text-sm md:text-lg md:text-xl leading-relaxed">
                  Your wristband for {eventName} is ready
                </p>
              </div>
            </div>
          </section>
              ...
              <div className="bg-white rounded-lg shadow-lg p-8 md:p-12">
                ...
                <div className="bg-gray-50 rounded-lg p-6 mb-6">
                  ...
                  <div className="space-y-3">
                    ...
                    {ticketNumber && (
                      <div className="flex justify-between">
                        <span className="text-gray-600">Wristband Number:</span>
                ...
                <div className="bg-blue-50 rounded-lg p-6 mb-6">
                  ...
                  <ul className="space-y-2 text-gray-700">
                    <li>
                      • Save your wristband number:{" "}
                      <strong className="font-mono">{ticketNumber}</strong>
                    </li>
                    <li>
                      • You&apos;ll receive a confirmation email shortly
                    </li>
                    <li>• Pickup location at Weldon (date TBA)</li>
                    <li>• Skip the line - straight inside!</li>
                    <li>• This event is 19+</li>
                  </ul>
                </div>
...
export default function LostLoveTicketConfirmPage() {
  return (
    <Suspense fallback={
      <>
        <Navbar />
