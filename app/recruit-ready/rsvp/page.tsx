...
export default function RecruitReadyRsvpPage() {
  ...
  useEffect(() => {
    const fetchEventDetails = async () => {
      if (!user?.userId) {
        ...
        router.push(`/sign-in?redirect=${encodeURIComponent("/recruit-ready/rsvp")}`);
        return;
      }
    ...
    try {
      ...
      if (data.success) {
        if (data.attending === false) {
          ...
          router.push(
            `/recruit-ready/ticket/confirm?ticketNumber=${data.ticketNumber}&eventId=${eventId}&rsvp=true`
          );
        }
      }
    } catch (error) {
                  ...
                  <div className="mb-6 p-6 bg-blue-50 rounded-lg border-2 border-blue-500">
                    <div className="text-center">
                      ...
                      <button
                        onClick={() =>
                          router.push(
                            `/recruit-ready/ticket/confirm?ticketNumber=${existingTicketNumber}&eventId=${eventId}`
                          )
                        }
