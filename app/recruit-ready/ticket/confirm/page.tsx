...
function TicketConfirmContent() {
  ...
  const formatDateTimeForGoogle = (dateStr: string, timeStr: string) => {
    try {
      // Parse date (e.g., "Monday, January 19th, 2026")
      const parts = dateStr.split(" ");
      const month = parts[1];
      const dayStr = parts[2].replace(",", "").replace(/st|nd|rd|th/g, ""); // Remove ordinal suffixes
      const year = parts[3];
      
      // Parse time - handle ranges like "3:30 PM - 6:30 PM" by taking start time
      let timePart: string;
      let modifier: string;
      if (timeStr.includes(" - ")) {
  ...
  const fetchTicketDetails = useCallback(async (userId: string, eventId: string) => {
    try {
      ...
      if (eventResponse.ok) {
        ...
        setEventName(eventData.name || "Recruit Ready");
        setEventDate(eventData.date || "");
        setEventTime(eventData.time || "");
        setEventLocation(eventData.location || "");

        // Generate Google Calendar link (using the backend function for consistency)
        if (eventData.date && eventData.time && eventData.location) {
          // For now, we'll generate it on the backend via API call or use a simpler approach
          // The backend generateGoogleCalendarLink handles date/time parsing better
          // We'll construct it here with proper parsing
          try {
...
export default function RecruitReadyTicketConfirmPage() {
  return (
    <Suspense fallback={
      <>
        <Navbar />
