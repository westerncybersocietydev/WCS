...
const BecomeVIP: React.FC<BecomeVIPProps> = ({
  isOpen,
  onClose,
  onComplete,
}) => {
          ...
          {paymentOpen ? (
            ...
            <>
              <div>
                <ul className="space-y-2 ml-8">
                  {VIP.map((benefit, index) => (
                    <li
                      key={index}
                      className="flex font-bold items-center text-gray-700 text-md"
                    >
              ...
              <p
                ...
                style={{ fontSize: "10px" }}
              >
                You will be charged a one-time payment of $15, which will cover
                your membership for the remainder of the academic year. The next
                payment will be due in September of the following year.
              </p>
            </>
          )}
        </div>
